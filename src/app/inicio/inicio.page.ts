...
import {
  Publicacion,
  PublicacionService,
} from 'src/services/publicacion.service';
...
import {
  ...
  BannerAdPosition,
  BannerAdSize,
} from '@capacitor-community/admob/dist/esm/banner';
import { initializeApp } from 'firebase/app';


// Creamos una interfaz extendida que incluye la propiedad 'comuna'
interface PublicacionConComuna extends Publicacion {
  comuna?: string;  // Propiedad que agregamos de manera temporal
}
...
export class inicioPage implements OnInit {
  ...
  filteredPublicaciones: PublicacionConComuna[] = [];
  searchTerm: string = '';
  ...
  async cargarData() {
    ...
    this.publicaciones$ =
      this.publicacionService.obtenerTodasLasPublicaciones();
    this.publicaciones$.subscribe((publicaciones) => {
      ...
      this.filteredPublicaciones.forEach((publicacion) => {
        if (publicacion.coordenadas) {
          ...
          this.obtenerComuna(lat, lng, publicacion);
        }
      });
    });
  ...
  filterPublicaciones() {
    ...
    this.publicaciones$.subscribe((publicaciones) => {
      if (!term) {
        ...
        this.filteredPublicaciones = publicaciones.filter(
          (pub) =>
  ...
  // Función que acepta las coordenadas lat, lng y obtiene la comuna
  async obtenerComuna(lat: number, lng: number, publicacion: any) {
    try {
      // Llamar al servicio de geocodificación para obtener la comuna
      const comuna = await this.geocodingService.obtenerComunaDesdeLatLng(lat, lng);
      publicacion.comuna = comuna || 'Comuna desconocida';  // Asignar la comuna a la publicación
    } catch (error) {
      console.error('Error al obtener la comuna:', error);
      publicacion.comuna = 'Error al obtener la comuna'; // En caso de error, asignar un mensaje
    }
  }
